// PROTOTYPE H14 (from H05) — the red pen. Margin notes that sit next to what they point at (desktop) or between the blocks (phone),
...
const path = (d: string, i = 0) => `<path d="${d}" class="h14-s" pathLength="1" style="--i:${i}"/>`;
...
export function createInk(sheet: HTMLElement, notes: InkNote[], marks: InkMark[], isDesk: () => boolean) {
  ...
  function draw() {
    ...
    notes.forEach((note, n) => {
      ...
      out += `<g class="h14-mark${on.has(note.id) ? ' is-on' : ''}" data-mark="${note.id}">${m ? path(m, 0) : ''}${a ? path(a, 1) : ''}</g>`;
    });
    marks.forEach((mark, n) => {
      ...
      out += `<g class="h14-mark${on.has(mark.id) ? ' is-on' : ''}" data-mark="${mark.id}">${path(d)}</g>`;
    });
    svg.innerHTML = out;
  }
